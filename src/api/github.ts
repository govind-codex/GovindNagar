...
export const getContributedOrganizations = cache(
  async (username: string): Promise<ContributedOrganization[]> => {
    ...
    const repos = (json.data.user.repositoriesContributedTo?.nodes || []).filter(Boolean);
...
export const getDetailedActivity = cache(
  async (username: string): Promise<DetailedActivity> => {
    ...
    const repos = (userData.repositoriesContributedTo?.nodes || []).filter(Boolean);
...
export const getCachedContributions = cache(
  async (username: string): Promise<Contributions> => {
    ...
    const contributionsUrl = `https://github-contributions-api.jogruber.de/v4/govind-codex`;
    ...
    const repos = (githubData.data.user.repositories?.nodes || []).filter(Boolean);
    const totalStars = repos.reduce(
      (acc: number, r: any) => acc + (r?.stargazerCount || 0),
      0,
    );
    const totalForks = repos.reduce(
      (acc: number, r: any) => acc + (r?.forkCount || 0),
      0,
    );
...
function pushRelease(
  ...
  for (const r of repos) {
    if (!r || !r.releases?.nodes) continue;
    for (const rel of r.releases.nodes) {
      if (!rel || !rel.publishedAt) continue;
...
async function fetchHeroOrbitPayload(username: string): Promise<HeroOrbitData> {
  ...
  const repos: RawRepoNode[] = (user.repositories?.nodes ?? []).filter(Boolean);
  const collection = user.contributionsCollection ?? {};
  const starred: RawStarredRepo[] = (user.starredRepositories?.nodes ?? []).filter(Boolean);

  const ossRepos = repos.filter((r) => r && r.stargazerCount >= 1).length;
