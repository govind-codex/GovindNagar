  ...
  return (
    <StyleSwap swapKey={selectedStyle}>
      {selectedStyle === "minimal" ? (
        <MinimalBlog posts={posts} now={now} blogUrl={blogUrl} />
      ) : selectedStyle === "static" ? (
        <StaticBlog posts={posts} now={now} blogUrl={blogUrl} />
      ) : selectedStyle === "story" ? (
        <StoryBlog posts={posts} now={now} blogUrl={blogUrl} />
      ) : (
        <DynamicBlog posts={posts} now={now} blogUrl={blogUrl} />
      )}
    </StyleSwap>
  );
}

function MinimalBlog({ posts, now, blogUrl }: Props) {
  return (
      ...
      <div className="mx-auto max-w-3xl px-6 py-16 md:py-24">
        ...
        {posts.length === 0 ? (
          <BlurFade delay={BLUR_FADE_DELAY * 2}>
            <p className="text-sm font-mono text-muted-foreground py-12 text-center border border-dashed border-border/50 rounded-xl">
...
function StaticBlog({ posts, now, blogUrl }: Props) {
  return (
    <main className="min-h-screen w-full relative overflow-hidden">
      ...
      <div className="mx-auto max-w-5xl px-6 md:px-12 py-24">
        ...
        {posts.length === 0 ? (
          <BlurFade delay={BLUR_FADE_DELAY * 2}>
            <p className="text-center font-mono text-muted-foreground py-16 border border-dashed border-border/50 rounded-2xl">
            ...
            <div className="flex justify-center mx-auto mt-24 pt-16 border-t border-border/50">
              <GlowFillButton icon={ArrowUpRight}>
                <Link
                  ...
                  href={blogUrl}
                >
                  <BookOpen className="size-5 inline-block mr-2" />
                  Explore Articles
                </Link>
              </GlowFillButton>
            </div>
...
function StoryBlog({ posts, blogUrl }: Props) {
  return (
    <main className="mx-auto w-full max-w-3xl px-6 pb-24 pt-28 md:pt-36">
      <StoryReveal>
        ...
        <p className="mt-5 max-w-xl text-base leading-relaxed text-muted-foreground">
          Everything I have taken the time to write down properly. Distributed systems, frontend architecture, and whatever I
          happened to be wrestling with that week.
        </p>
      </StoryReveal>

      {posts.length === 0 ? (
        <StoryReveal className="mt-14" delay={0.05}>
          <p className="font-mono text-sm text-muted-foreground py-12 text-center border-b border-border/40">
...
function DynamicBlog({ posts, now, blogUrl }: Props) {
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden">
      ...
      <div className="mx-auto max-w-6xl px-6 py-12 md:py-16">
        {posts.length === 0 ? (
          <div className="py-20 text-center font-mono text-sm text-muted-foreground border border-dashed border-border/50 rounded-2xl">
            No blog posts published yet. Check back soon!
          </div>
        ) : (
          <>
        ...
        {blogUrl && (
          ...
          >
            <GlowFillButton icon={ArrowUpRight}>
              <Link target="_blank" rel="noreferrer noopener" href={blogUrl}>
                <BookOpen className="size-5 inline-block mr-2" />
                Explore Articles
              </Link>
            </GlowFillButton>
